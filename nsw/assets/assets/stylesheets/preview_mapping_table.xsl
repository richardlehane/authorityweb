...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  xmlns:exslt="http://exslt.org/common" xmlns:rda="http://www.records.nsw.gov.au/schemas/RDA"
  version="1.0">
  <xsl:output method="xml" />
  <xsl:include href="include/linking_table.xsl" />
  <xsl:include href="include/html_header.xsl" />
  <!-- if you want to always link to the same previous authority set a default prefix (e.g. GDA10)
  on the variable below-->
  ...
  <xsl:template match="rda:Authority">
    ...
    <html>
      ...
      <body>
        <H1 align="center">Mapping table</H1>
        <table width="100%">
          <tr>
            <td>
              <b>Old reference</b>
          ...
          <xsl:for-each select="exslt:node-set($linked_terms)/links">
            <xsl:for-each select="link_term">
              ...
              <tr>
                <td>
                  <xsl:choose>
                    <xsl:when test="old_ref/f"><xsl:if test="old_ref/prefix">
                        <xsl:value-of select="concat(old_ref/prefix, ' ')" />
                      </xsl:if><xsl:value-of
                        select="old_ref/f" />.<xsl:value-of select="old_ref/a" /><xsl:if
                        test="old_ref/c">.<xsl:value-of select="old_ref/c" /></xsl:if></xsl:when>
                    <xsl:otherwise>
