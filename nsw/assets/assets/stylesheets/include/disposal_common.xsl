...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="$disposals/rda:DisposalCondition='Authorised'">
                        <xsl:call-template name="standard_action">
                            <xsl:with-param name="disposal"
                                            select="$disposals[rda:DisposalCondition='Authorised']" />
                        </xsl:call-template>
                    </xsl:when>
                    ...
                    <xsl:when test="$disposals/rda:DisposalCondition='If longer'">
                        <xsl:call-template name="multiple_action">
                            <xsl:with-param name="disposals"
                                            select="$disposals[rda:DisposalCondition='If longer']" />
                        </xsl:call-template>
                    </xsl:when>
                    <xsl:when test="$disposals/rda:DisposalCondition='If shorter'">
                        <xsl:call-template name="multiple_action">
                            <xsl:with-param name="disposals"
                                            select="$disposals[rda:DisposalCondition='If shorter']" />
                        </xsl:call-template>
                    </xsl:when>
                    <xsl:otherwise>
    ...
    <xsl:template name="retention_string">
        ...
        <xsl:choose>
            <xsl:when test="$disposal/rda:RetentionPeriod">
                <xsl:variable name="initialunit"
                              select="$disposal/rda:RetentionPeriod/@unit" />
                <xsl:choose>
    ...
    <xsl:template name="simple">
        ...
        <xsl:variable name="trigger_list"
                      select="'//action completed/superseded/reference use ceases/administrative or reference use ceases/'" />
        <xsl:choose>
            <xsl:when test="$disposals/rda:DisposalCondition='Automated'">
                <xsl:call-template name="router">
                    <xsl:with-param name="disposal"
                                    select="$disposals[rda:DisposalCondition='Automated']" />
            ...
            <xsl:when test="$disposals[not(rda:DisposalCondition='Authorised')]">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:call-template name="router">
                            <xsl:with-param name="disposal"
                                            select="$disposals[not(rda:DisposalCondition='Authorised')]" />
                            <xsl:with-param name="action"
                                            select="$action" />
                        </xsl:call-template>
                    </xsl:otherwise>
    ...
    <xsl:template name="single_action">
        ...
        <xsl:call-template name="check_action">
            <xsl:with-param name="subsequent"
                            select="$disposals[rda:DisposalAction][position() > 1]" />
            <xsl:with-param name="first"
                            select="$disposals[rda:DisposalAction][1]" />
        </xsl:call-template>
    </xsl:template>
