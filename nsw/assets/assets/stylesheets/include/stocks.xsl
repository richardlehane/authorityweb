...
<xsl:stylesheet version="1.0"
    ...
    <xsl:variable name="ARNO">
        ...
        <xsl:value-of select="(rda:Authority/rda:ID[@control='AR'])[last()]" />
    </xsl:variable>
    <xsl:variable name="RDANO">
        <xsl:choose>
            <xsl:when
                test="rda:Authority/rda:ID[@control='FA' or @control='GA' or @control='DA' or @control='GDA' or @control='DR']">
                <xsl:for-each
                    select="rda:Authority/rda:ID[@control='FA' or @control='GA' or @control='DA' or @control='GDA' or @control='DR']">
    ...
    <xsl:variable name="SRFILENO">
        <xsl:value-of select="(rda:Authority/rda:ID[@control='SRFileNo'])[last()]" />
    </xsl:variable>
    ...
    <!-- list of agencies: either to whom the authority was issued or who were involved in drafting
    it -->
    <xsl:variable name="AGENCY_NAMES">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:for-each
                    select="rda:Authority/rda:Status/rda:Draft[not(rda:Agency = preceding-sibling::rda:Draft/rda:Agency)]">
                    <xsl:if test="position() >1">
                        <xsl:choose>
                            <xsl:when test="position()=last()">
                                <xsl:text> and </xsl:text>
    ...
    <xsl:variable name="ISSUE_VERSION">
        <xsl:choose>
            <xsl:when test="rda:Authority/rda:Status/rda:Amended">
                <xsl:value-of select="(rda:Authority/rda:Status/rda:Amended)[last()]/@version" />
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:variable name="ISSUE_DATE">
        <xsl:choose>
            <xsl:when test="rda:Authority/rda:Status/rda:Amended">
                <xsl:value-of select="(rda:Authority/rda:Status/rda:Amended)[last()]/rda:Date" />
            </xsl:when>
            <xsl:otherwise>
                <xsl:choose>
    ...
    <xsl:variable name="ISSUE_MONTH_TEXT">
