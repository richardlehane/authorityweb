...
<xsl:stylesheet version="1.0"
    ...
    <!-- turn a date into 10 Dec 1920-->
    <xsl:template name="day_month_year">
        <xsl:param name="date" />
        ...
        <xsl:value-of
            select="concat(substring($date, 9, 2), ' ', $month, ' ', substring($date, 1, 4))" />
    </xsl:template>
            ...
            <xsl:otherwise>
                <xsl:choose>
                    ...
                    <xsl:when test="$disposals[1]/rda:DisposalAction='Required as State archives'">
                        <xsl:choose>
                            <xsl:when
                                test="$disposals[1]/rda:RetentionPeriod or $disposals[1]/rda:DisposalTrigger">
                                <xsl:text>true</xsl:text>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:call-template name="testcustody">
    ...
    <!-- this traverses up the node tree and returns the first RDANO found -useful for authorities
    with multiple RDANOs -->
    ...
    <xsl:template name="get_id">
        ...
        <xsl:choose>
            <xsl:when
                test="$node/rda:ID[@control='FA' or @control='GA' or @control='DA' or @control='GDA' or @control='DR']">
                <xsl:for-each
                    select="$node/rda:ID[@control='FA' or @control='GA' or @control='DA' or @control='GDA' or @control='DR']">
    ...
    <xsl:template name="titleise">
        ...
        <xsl:choose>
            <xsl:when test="contains($title, '&amp;')">
                <xsl:value-of
                    select="translate(concat(substring-before($title, '&amp;'), 'and', substring-after($title, '&amp;')), $illegal, $clean)" />
            </xsl:when>
            <xsl:otherwise>
